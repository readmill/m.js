 ...
 * never created manually but build using the m.module() function. This
 * is based on a Backbone.View as it provides the basics for settings up and
 * tearing down DOM elements. In addition it receives a sandbox object which is
 * it's communication with the rest of the page.
 */
(function (m, jQuery, Backbone, _) {
  var Module = Backbone.View.extend({
     ...
     */
    remove: function () {
      Module.__super__.remove.apply(this, arguments);
      if (this.sandbox && typeof this.sandbox.publish === 'function') {
  ...
  var ModuleFactory = m.create({
    ...
    build: function (options) {
      ...
      // can get the name output in the console. Otherwise all objects have the
      // name "child" which is defined by Backbone. We have to do this each
      // time an object is created to ensure unique constructors.
      var cloned = _.clone(this.properties);
      var className = jQuery.camelCase(this.type) + 'Module';
...
})(this.m = this.m || {}, this.jQuery, this.Backbone, this._);
